...
def main():
    ...
    **专业的亚马逊A+页面内容生成工具**
    ...
    🎯 **模块化制作**：从12个专业模块中选择，上传产品素材，AI智能生成符合亚马逊规范的A+页面内容
    
    💡 **卖点分析**：上传产品图片，AI分析产品特点并生成营销建议和文案
    """)
...
def render_selling_points_analysis():
    ...
    # 功能说明 - 更清晰的描述
    st.markdown("""
    **功能说明**：上传产品图片，AI智能分析产品的视觉特点、核心卖点和营销价值，为您的A+页面和产品描述提供专业建议。
                        ...
                        for j in range(2):
                            if i + j < len(uploaded_files):
                                with cols[j]:
                                    ...
                                    st.image(image, caption=f"图片 {i+j+1}", use_column_width=True)
...
def render_module_selection_step():
    ...
    **模块化A+制作说明**：
    ...
    从12个专业模块中选择您需要的内容类型，每个模块专注于产品展示的特定方面：
...
def render_material_upload_step():
    ...
    **素材上传说明**：
    ...
    为您选择的 {len(selected_modules)} 个模块上传相关素材，AI将根据这些素材生成专业的A+页面内容。
    
    **支持的素材类型**：
    - **图片文件**：产品照片、使用场景图、细节图等 (JPG/PNG/WebP，最大10MB)
    - **文档文件**：产品说明书、规格表、认证文件等 (PDF/DOC/TXT，最大5MB)  
    - **文本内容**：产品描述、卖点说明、技术参数等
    - **自定义提示**：特殊要求或生成指导
...
def render_generation_step():
    ...
    **内容生成说明**：
    ...
    AI将根据您上传的素材为 {len(selected_modules)} 个模块生成专业的A+页面内容。
    
    **生成特点**：
    - 符合亚马逊A+页面规范 (600x450像素，4:3比例)
    - 信息密集型设计，突出产品特点
    - 专业视觉布局，提升转化率
    - 支持多语言文本覆盖
    """)
...
def render_preview_step():
    ...
    **预览和管理说明**：
    ...
    共生成了 {len(generated_modules)} 个A+模块，您可以：
