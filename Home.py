...
def get_simple_news():
    ...
    return [
        {
            'title': '🎯 Amazon Q4政策更新提醒',
            'desc': '年底合规检查，注意产品安全标准',
            'source': '政策'
        },
        {
            'title': '💰 FBA费用优化建议',
...
# 显示简化的资讯模块
with st.expander("📰 Amazon运营资讯", expanded=True):
    st.caption("� 实用的A0mazon运营知识和政策更新")
    
    news_list = get_simple_news()
    ...
    for i, news in enumerate(news_list):
        ...
        with target_col:
            ...
            ">
                ...
                <h4 style="margin: 0 0 8px 0; color: #1f2937; font-size: 0.95rem;">
                    {news['title']}
                </h4>
    ...
    # 简单的刷新按钮
    if st.button("🔄 刷新资讯", use_container_width=True, key="refresh_simple_news"):
        st.cache_data.clear()
        st.success("✅ 资讯已刷新")
