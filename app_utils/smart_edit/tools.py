...
class SmartEditHistory:
    """[Smart Edit 专属] 历史记录管理器"""
    def __init__(self, key="smart_edit_history", max_len=20):
        self.key = key
...
def render_history_sidebar(history_manager):
    ...
    with st.expander("🕒 历史记录 (History)", expanded=False):
        ...
        if items:
            if st.button("🗑️ 清空所有记录", key="se_clear_all", use_container_width=True):
        ...
        for item in items:
            with st.container(border=True):
                ...
                with col_info:
                    ...
                    with b1:
                        if st.button("🔍", key=f"se_zoom_{item['id']}", help="预览"):
                            show_image_modal(item['image'], item['source'])
                    with b2:
                        final_bytes, mime = process_image_for_download(item['image'], format="JPEG")
                        st.download_button("📥", data=final_bytes, file_name=f"hist_{item['id']}.jpg", mime=mime, key=f"se_dl_{item['id']}")
                    with b3:
                        if st.button("🗑️", key=f"se_del_{item['id']}", help="删除"):
